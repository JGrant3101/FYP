...
title('Floor Profile')
xlabel('Distance longitudinally along the floor (m)')
ylabel('Distance from the floor to the road (m)')
%axis equal
...
vCar = 300; % This is in kph will convert to m/s
vCar = (vCar * 10^3)/3600;
...
% Mass flow loss fraction when stalling
Mflowloss = 0.7;
...
for i = 2:FloorIndices
    ...
    if airMachs(i) > 0.99
        airspeeds(i) = airspeeds(i) * (1 - Mflowloss);
        % Finding the new air temp
        airTemps(i) = constant1 - (((gamma - 1) * airspeeds(i)^2) / (2 * gamma * R));
        % Finding the new mach number
        Sos(i) = sqrt(gamma * R * airTemps(i));
...
vCararray = linspace(50, 350, 121)';
Diff2staticarray = linspace(0, -0.075, 151)';

% Creating an empty downforce array
DownforceVals = zeros(length(vCararray), length(Diff2staticarray));
FrontDownforceVals = zeros(length(vCararray), length(Diff2staticarray));
RearDownforceVals = zeros(length(vCararray), length(Diff2staticarray));
Front2RearSplit = zeros(length(vCararray), length(Diff2staticarray));
...
for i = 1:length(vCararray)
    for j = 1:length(Diff2staticarray)
        ...
        for k = 2:FloorIndices
            ...
            % correct it as a normal shock would have occurred
            if airMachs(k) > 0.99
                airspeeds(k) = airspeeds(i) * (1 - Mflowloss);
                % Finding the new air temp
                airTemps(k) = constant1 - (((gamma - 1) * airspeeds(k)^2) / (2 * gamma * R));
                % Finding the new mach number
                Sos(k) = sqrt(gamma * R * airTemps(k));
        ...
        % FrontDownforceVals(i, j) = DWFFconstant + DWFFgrad .* vCar.^2 .* (0.0969 - FloorProfile(1));
        % RearDownforceVals(i, j) = DownforceVals(i, j) - FrontDownforceVals(i, j);

        % Calculate the force on each element
    Forces = deltaP * FloorWidth * (FloorArray(2) - FloorArray(1));
    TotalForce = sum(Forces);
    
    % Calculate the distance from each element to CoG
    distCoG = FloorArray - 1.38;
    distCoG = distCoG * -1;
    
    % Calculating the overall moment at CoG due to aero forces, this 
    TotalMoment = sum(Forces .* distCoG);
    
    % Calculating front downforce
    FrontDownforce = (TotalForce + (1.38/1.62) * TotalMoment) / (1 + (1.38/1.62));
    RearDownforce = TotalForce - FrontDownforce;

    FrontDownforceVals(i, j) = FrontDownforce;
    RearDownforceVals(i, j) = RearDownforce;
    Front2RearSplit(i, j) = FrontDownforce / (FrontDownforce + RearDownforce);
    if abs(Front2RearSplit(i, j)) > 1 
        Front2RearSplit(i, j) = NaN;
    end
    end
end
...
figure
scatter3(avgRideHeightmesh, vCararraymesh, Front2RearSplit, '.')
ylabel('vCar (kph)')
xlabel('Average ride height (m)')
zlabel('Front to rear downforce split')
...
% Fitting
fittedDWFfunc = fit([avgRideHeightcolumn vCarcolumn], DownforceValscolumn, 'poly35', 'normalize', 'on');
...
predictedvals = fittedDWFfunc.p00 + fittedDWFfunc.p10.*x + fittedDWFfunc.p01.*y + fittedDWFfunc.p20.*x.^2 + fittedDWFfunc.p11.*x.*y + fittedDWFfunc.p02.*y.^2 ... 
    ...
    ... % + fittedDWFfunc.p41.*x.^4.*y ...
    + fittedDWFfunc.p32.*x.^3.*y.^2 + fittedDWFfunc.p23.*x.^2.*y.^3 + fittedDWFfunc.p14.*x.*y.^4 + fittedDWFfunc.p05.*y.^5;
...
surf(avgRideHeightmesh, vCararraymesh, FrontDownforceVals)
...
xlabel('Average ride height (m)')
zlabel('Front to rear downforce split')
title('Mapped downforce from floor polynomial')
fontsize(gca, 20, 'points')
...
% Plotting all of the points where the model says that the floor is
% generating lift overall
% figure
% colormap(turbo)
% %scatter3(avgRideHeightmesh, vCararraymesh, DownforceVals, '.')
% ylabel('vCar (kph)')
...
% Want to for a range of car speeds plot the relationship of downforce
% generated with respect to ride height
% for i = 1:10
%     figure 
%     tiledlayout(4, 3)
%     for j = 1:12
%         nexttile
%         plot(avgRideHeightmesh(j + (i-1)*12, :), predictedvalsmesh(j + (i-1)*12, :))
%         title(['Speed = ', num2str(vCararraymesh((j + (i-1)*12), 1))])
%         hold on
%         plot(avgRideHeightmesh(j + (i-1)*12, :), FrontDownforceVals(j + (i-1)*12, :))
%     end
% end
return
%% Saving variables
% Saving variables produced by this script to be later used in the overall
% car model
avgRHmean = mean(avgRideHeightcolumn);
avgRHstd = std(avgRideHeightcolumn);
vCarmean = mean(vCarcolumn);
vCarstd = std(vCarcolumn);
save("C:\Users\joegr\Repos\FYP\MyCode\Porpoising\FullCarModel\AeroValues", "avgRHmean", "avgRHstd", "vCarmean", "vCarstd", "fittedDWFfunc")