function rhs  = SuspensionWithTimeBumpyRoad(t, x, p, RoadProfile)
...
% Find the disturbance from the road
RoadDisturb = interp1(RoadProfile(:, 1), RoadProfile(:, 2), t);
...
rhs(4, :) = (Ks./Mu) .* Zs - ((Ks./Mu) + (Kt./Mu)) .* Zu + (Cs./Mu) .* Phis - (Cs./Mu) .* Phiu + RoadDisturb * Kt/Mu;
