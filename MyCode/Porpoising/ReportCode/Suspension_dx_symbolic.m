%% Defining the Jacobian for the suspension system of the car w.r.t the x vector NEW.
function J = Suspension_dx_symbolic(x, p)

% Firstly define the variable terms
% Sprung mass vertical displacement
Zs = x(1, :);
% Unsprung mass vertical displacement
Zu = x(2, :);
% Sprung mass vertical velocity
Phis = x(3, :);
% Unsprung mass vertical velocity
Phiu = x(4, :);

% Then defining the constant terms
% Sprung mass
Ms = p(1, :);
% Unsprung mass
Mu = p(2, :);
% Suspension stiffness
Ks = p(3, :);
% Suspension damping
Cs = p(4, :);
% Tyre stiffness
Kt = p(5, :);
% Static ride height
H = p(6, :);
% Car speed
vCar = ((p(7, :).*10^3)./(60*60));
% Upper downforce elements multiplier
A = p(8, :);
% Mean in log normal function
mew = p(9, :);
% Standard deviation in log normal function
lamda = p(10, :);
% Scaling for Inverse Gaussian distribution
scaling = p(11, :);

% Calculating ride height, this is the static ride height plus the vertical
% displacements of both the sprung and unsprung mass.
h = H + x(1, :) + x(2, :);

% Initialising our Jacobian
% J = zeros(4, 4, numel(Zs));
J = sym('J', [4, 4]);

% Populating the non-zero terms with values
J(1, 1, :) = 0;
J(1, 2, :) = 0;
J(1, 3, :) = 1;
J(1, 4, :) = 0;

J(2, 1, :) = 0;
J(2, 2, :) = 0;
J(2, 3, :) = 0;
J(2, 4, :) = 1;

J(3, 1, :) = -(Ks./Ms) - (scaling./(Ms .* h.^2 .* lamda .* sqrt(2.*pi))) * exp((-(log(h) - mew).^2) ./ (2 * lamda.^2)) * ((mew - log(h))./(lamda.^2) - 1);
J(3, 2, :) = (Ks./Ms) - (scaling./(Ms .* h.^2 .* lamda .* sqrt(2.*pi))) * exp((-(log(h) - mew).^2) ./ (2 * lamda.^2)) * ((mew - log(h))./(lamda.^2) - 1);
J(3, 3, :) = -Cs ./ Ms;
J(3, 4, :) = Cs ./ Ms;

J(4, 1, :) = Ks ./ Mu;
J(4, 2, :) = -((Ks ./ Mu) + (Kt ./ Mu));
J(4, 3, :) = Cs ./ Mu;
J(4, 4, :) = -Cs ./ Mu;

end