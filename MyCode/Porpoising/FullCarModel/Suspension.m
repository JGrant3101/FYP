...
temp1 = zeros([4, 4, length(p(1, :))]);
temp2 = zeros([4, 1, length(p(1, :))]);
...
temp2(1, 1, :) = StaticFRH + 0.6 .* tan(StaticRake);
temp2(2, 1, :) = StaticRRH;
temp2(3, 1, :) = Ylow;
temp2(4, 1, :) = 0;

% Solving
a = zeros([1, length(temp2(1, 1, :))]);
b = zeros([1, length(temp2(1, 1, :))]);
c = zeros([1, length(temp2(1, 1, :))]);
d = zeros([1, length(temp2(1, 1, :))]);
for i = 1:length(temp2(1, 1, :))
    values = linsolve(temp1(:, :, i), temp2(:, 1, i));
    a(1, i) = values(1);
    b(1, i) = values(2);
    c(1, i) = values(3);
    d(1, i) = values(4);
