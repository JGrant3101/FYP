...
LF = p(5, :);
% Distnace from rear axle to CoG
LR = p(6, :);
...
vCarnormalised = (vCar - vCarmean) ./ vCarstd;
...
TotalDWFFloor = p00 + p10.*x + p01.*y + p20.*x.^2 + p11.*x.*y + p02.*y.^2 + p30.*x.^3 + p21.*x.^2.*y + p12.*x.*y.^2 + p03.*y.^3 + p40.*x.^4 ...
    + p31.*x.^3.*y + p22.*x.^2.*y.^2 + p13.*x.*y.^3 + p04.*y.^4 + p50.*x.^5 + p41.*x.^4.*y + p32.*x.^3.*y.^2 + p23.*x.^2.*y.^3 + p14.*x.*y.^4 + p05.*y.^5;

% Calculating the DWF on the front axle from the floor using the linear
% approximation
DWFFloorF = m .* vCar.^2 .* (StaticFRH - hF);
...
rhs(8, :) = (LR ./ Is) .* (KsR .* (Zs + LR .* THETAs - ZuR) + CsR .* (Phis + LR .* PhiTHETAs - PhiuR) + DWFR) ...
    - (LF ./ Is) .* (KsF .* (Zs - ZuF - LF .* THETAs) + CsF .* (Phis - PhiuF - LF .* PhiTHETAs) + DWFF);
