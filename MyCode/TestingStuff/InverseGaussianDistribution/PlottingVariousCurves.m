...
% 
% mews = linspace(0, 1, 101)';
% results = zeros(length(h), length(mews));
% for i = 1:length(mews)
%     results(:, i) = DWFFloorNew(h, mews(i), 2.4, 0.31*(500/9)^2);
% end
% 
% figure
% hold on
...
% c1 = [0, 0.447, 0.741];
% c2 = [0.85, 0.325, 0.098];
% c = [linspace(c1(1),c2(1),length(mews))', linspace(c1(2),c2(2),length(mews))', linspace(c1(3),c2(3),length(mews))'];
% for i = 1:length(mews)
...
% for i = 1:length(mews) 
%     legendlabels{i+1, 1} = "mew = " + num2str(mews(i));
% end
% legend(legendlabels)
